...
public class WaterNetwork {
	...
	private double leak(double pipeOpen) {
		return Math.max(pipeOpen, PIPE_LEAK);
	}
	...
	protected void simulationStep() {
		...
		for (int i = 0;i<pipes.length;i++) {
			...
			if (p.start instanceof Reservoir) {
				rhs.set(i,0, p.crossSection*leak(p.open)*FLOW_CONSTANT*((Reservoir)p.start).level());
			}
			else
			{
				m.set(i, pipes.length+p.start.ID, - p.crossSection*leak(p.open)*FLOW_CONSTANT);
				m.set(pipes.length+p.start.ID, i, -1); // node equilibrium
			}
			
			// level at pipe end
			if (p.end instanceof Reservoir) {
				rhs.set(i, 0, rhs.get(i,0) - p.crossSection*leak(p.open)*FLOW_CONSTANT*((Reservoir)p.end).level());
			}
			else
			{
				m.set(i, pipes.length+p.end.ID, p.crossSection*leak(p.open)*FLOW_CONSTANT);
	...
	private void setDefaultConfig3() {
		...
		//nodes[3] = new Reservoir(10, 0, 17, 30, new Point2D.Double(100,100));
	...
	private void setDefaultConfig6() {
		nodes = new NetworkNode[4];
		nodes[0] = new Reservoir(100, 100, 20, 25, new Point2D.Double(0,0));
		nodes[1] = new Reservoir(100, 0, 18, 20, new Point2D.Double(100,0));
		nodes[2] = new Reservoir(200, 0, 17, 19, new Point2D.Double(0,100));
		nodes[3] = new NetworkNode(new Point2D.Double(50, 50));
		pipes = new Pipe[3];
		pipes[0] = new Pipe(nodes[0],nodes[3], 10.0, 0);
		pipes[1] = new Pipe(nodes[1],nodes[3], 10.0, 0);
		pipes[2] = new Pipe(nodes[2],nodes[3], 10.0, 0);
	}
